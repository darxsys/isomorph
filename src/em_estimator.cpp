    ...
    if (!params.paired_end) {
        ...
        for (int n = 0; n < num_reads; ++n) {
            ...
            for (auto record : read->pi_x_n) {
                ...
                double insert_prob = prob_normal(params.insert_mean,
                                                 params.insert_stdev,
                                                 get<3>(record));
        ...
        do {
            ...
            for (int n = 0; n < num_reads; ++n) {
                ...
                for (int t = 0; t < read->pi_x_n.size(); ++t) {
                    ...
                    int i = get<0>(record);
                    int transcript_len = length(transcripts.seqs[i]);
                    pre_m_expressions[i] += read_posteriors[n][t] * expressions[i] /
                                            transcript_len / read_expect_sum;
                }
            }
